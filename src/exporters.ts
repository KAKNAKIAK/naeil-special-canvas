...
function renderSpecialSection(project: Project, section: Section) {
  if (section.type === 'caption-grid') {
    ...
    const figures = `<figure>${renderSpecialImage(project, section.mediaIds[0], '왼쪽 이미지')}${caption(0)}</figure><figure>${renderSpecialImage(project, section.mediaIds[1], '오른쪽 이미지')}${caption(1)}</figure>`
