...
function splitPair(value: string) { const [title = '', ...body] = value.split('|').map(part => part.trim()); return { title, body: body.join(' | ') } }
function splitTimeline(value: string) { const [time = '', title = '', ...body] = value.split('|').map(part => part.trim()); return { time, title, body: body.join(' | ') } }
...
function renderSpecialSection(project: Project, section: Section) {
  if (section.type === 'caption-grid') {
    const figures = section.items.map((caption, index) => `<figure>${renderSpecialImage(project, section.mediaIds[index], `이미지 ${index + 1}`)}<figcaption>${escapeHtml(caption)}</figcaption></figure>`).join('')
    return `<section id="${escapeHtml(section.id)}" class="block special-section caption-grid-block">${renderSpecialCopy(section, 'special-common-copy caption-grid-copy')}<div class="caption-grid">${figures}</div>${section.note ? `<p class="designer-note">DESIGN NOTE · ${escapeHtml(section.note)}</p>` : ''}</section>`
  }
...
export function standaloneHtml(project: Project) {
  const sections = project.sections.filter(s => !s.hidden).map(s => {
