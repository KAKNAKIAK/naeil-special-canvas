...
import { Archive, ArrowLeftRight, Baby, Bath, Blocks, CalendarDays, CarFront, Check, ChevronDown, ChevronUp, CircleHelp, Compass, Copy, Download, Dumbbell, ExternalLink, Eye, FileJson, FileText, Flag, Flower2, FolderOpen, GripVertical, HandHeart, Hotel, Image as ImageIcon, Layers, Link2, LogOut, MapPinned, Maximize2, Menu, Minus, MonitorUp, PanelsTopLeft, Plane, Plus, Redo2, Save, Settings2, ShoppingBag, Sparkles, Star, Table2, Trash2, Undo2, Upload, UsersRound, Utensils, Waves, X } from 'lucide-react'
...
export default function App() {
  ...
  function swapSectionMedia(sectionId: string, fromAssetId: string, toAssetId: string) { if (fromAssetId === toAssetId) return; let replacedFromLibrary = false; commit(draft => { const section = draft.sections.find(item => item.id === sectionId); if (!section) return; const boxes = normalizeLayoutBoxes(section); const targetBox = boxes.find(box => (box.kind === 'media' || box.kind === 'image') && (box.assetIds || []).includes(toAssetId)); if (!targetBox) return; const targetIds = targetBox.assetIds || []; replacedFromLibrary = !targetIds.includes(fromAssetId); const nextIds = replacedFromLibrary ? uniqueIds(targetIds.map(id => id === toAssetId ? fromAssetId : id)) : targetIds.map(id => id === fromAssetId ? toAssetId : id === toAssetId ? fromAssetId : id); const nextBoxes = boxes.map(box => box.id === targetBox.id ? { ...box, assetIds: nextIds } : box); section.layoutBoxes = nextBoxes; section.mediaIds = mediaIdsFromBoxes(nextBoxes); if (section.mediaLayout === 'custom') section.mediaLayoutItems = normalizeCustomLayout(section.mediaIds, section.mediaLayoutItems) }); setToast(replacedFromLibrary ? '라이브러리 이미지로 교체했습니다.' : '이미지 위치를 바꿨습니다.') }
...
function ImageLibraryWithCleanup({ assets, linkedAssetIds, onClearUnused, ...props }: { assets: MediaAsset[]; linkedAssetIds: string[]; onClearUnused: () => void; selected?: Section; selectedBox?: BlockBox; selectedSpecialMediaIndex: number | null; onUpload: () => void; onAddUrl: (url: string) => void; onConnect: (id: string) => void }) {
  const libraryRef = useRef<HTMLDivElement>(null)
  const unusedCount = assets.filter(asset => !linkedAssetIds.includes(asset.id)).length
  useEffect(() => {
    const buttons = libraryRef.current?.querySelectorAll<HTMLButtonElement>('.asset-grid article > button') || []
    const listeners = Array.from(buttons).map((button, index) => {
      const asset = assets[index]
      const onDragStart = (event: DragEvent) => { if (!asset || !event.dataTransfer) return; event.dataTransfer.effectAllowed = 'copy'; event.dataTransfer.setData('text/plain', asset.id); event.dataTransfer.setData('application/x-naeil-library-asset', asset.id) }
      button.draggable = true
      button.addEventListener('dragstart', onDragStart)
      return () => { button.draggable = false; button.removeEventListener('dragstart', onDragStart) }
    })
    return () => listeners.forEach(remove => remove())
  }, [assets])
  return <div ref={libraryRef} className="image-library-with-cleanup"><button className="clear-unused-assets" type="button" disabled={!unusedCount} onClick={onClearUnused}><Trash2/> 연결되지 않은 이미지 전체 삭제{unusedCount ? ` (${unusedCount})` : ''}</button><ImageLibrary assets={assets} {...props}/></div>
}
...
function SectionView({ section, assets, selected, readOnly, activeBoxId, activeItemIndex, activeSpecialMediaIndex, index, total, onSelect, onSelectBox, onSelectItem, onSelectSpecialMedia, onClearTimelineMedia, onSetTimelineHeroVisible, onDeleteTimelineDay, onSectionChange, onBoxChange, onUpdateItem, onAddFlowBox, onMoveFlowBox, onDeleteFlowBox, onAddItem, onAddTimelineDay, onInsertItem, onDuplicateItem, onMoveItem, onReorderIconCard, onDeleteItem, onToggleMenuItemLayout, onMove, onDuplicate, onDelete, onSwapMedia }: { section: Section; assets: MediaAsset[]; selected: boolean; readOnly: boolean; activeBoxId: string; activeItemIndex: number | null; activeSpecialMediaIndex: number | null; index: number; total: number; onSelect: () => void; onSelectBox: (id: string) => void; onSelectItem: (index: number) => void; onSelectSpecialMedia: (index: number) => void; onClearTimelineMedia: (sectionId: string, mediaIndex: number) => void; onSetTimelineHeroVisible: (sectionId: string, visible: boolean) => void; onDeleteTimelineDay: (sectionId: string, dayIndex: number) => void; onSectionChange: (patch: Partial<Section>) => void; onBoxChange: (boxId: string, patch: Partial<BlockBox>) => void; onUpdateItem: (index: number, value: string | string[] | Partial<IconCardItem>) => void; onAddFlowBox: (sectionId: string, kind: 'text' | 'image') => void; onMoveFlowBox: (sectionId: string, boxId: string, direction: -1 | 1) => void; onDeleteFlowBox: (sectionId: string, boxId: string) => void; onAddItem: (sectionId: string) => void; onAddTimelineDay: (sectionId: string) => void; onInsertItem: (sectionId: string, index: number, placement: 'above' | 'below') => void; onDuplicateItem: (sectionId: string, index: number) => void; onMoveItem: (sectionId: string, index: number, direction: -1 | 1) => void; onReorderIconCard: (sectionId: string, fromIndex: number, insertionIndex: number) => void; onDeleteItem: (sectionId: string, index: number) => void; onToggleMenuItemLayout: (sectionId: string, index: number) => void; onMove: (id: string, d: -1 | 1) => void; onDuplicate: (id: string) => void; onDelete: (id: string) => void; onSwapMedia: (sectionId: string, fromAssetId: string, toAssetId: string) => void }) {
