...
function ImageLibrary({ assets, selected, selectedBox, selectedSpecialMediaIndex, onUpload, onAddUrl, onAddGettyImage, onConnect }: { assets: MediaAsset[]; selected?: Section; selectedBox?: BlockBox; selectedSpecialMediaIndex: number | null; onUpload: () => void; onAddUrl: (url: string) => void; onAddGettyImage: (contentId: string) => Promise<boolean>; onConnect: (id: string) => void }) {
  ...
  return <>
    ...
    {!assets.length ? <div className="empty-state"><ImageIcon/><p>이미지를 올리면 이곳에서<br/>선택 이미지 자리에 바로 연결할 수 있습니다.</p></div> : <div className="asset-grid">{assets.map(asset => <article key={asset.id} className={connectedIds.includes(asset.id) ? 'connected' : ''}><button title={asset.src.startsWith('http') ? asset.src : asset.name} aria-label={`${asset.name} 연결 또는 해제`} onClick={() => onConnect(asset.id)}><img src={asset.src} alt={asset.alt || asset.name}/>{connectedIds.includes(asset.id) && <i><Check/></i>}</button></article>)}</div>}
  </>
}
