...
function splitPair(value: string) {
  const [title = '', ...body] = value.split('|').map(part => part.trim())
  return { title, body: body.join(' | ') }
}

function splitTimeline(value: string) {
  const [time = '', title = '', ...body] = value.split('|').map(part => part.trim())
  return { time, title, body: body.join(' | ') }
}
...
function ImageLibrary({ assets, selected, selectedBox, selectedSpecialMediaIndex, onUpload, onAddUrl, onConnect }: { assets: MediaAsset[]; selected?: Section; selectedBox?: BlockBox; selectedSpecialMediaIndex: number | null; onUpload: () => void; onAddUrl: (url: string) => void; onConnect: (id: string) => void }) { const [imageUrl, setImageUrl] = useState(''); const [librariesOpen, setLibrariesOpen] = useState(false); const isReference = Boolean(selected && REFERENCE_LAYOUT_TYPES.has(selected.type)); const connectedIds = isReference && selectedSpecialMediaIndex !== null ? [selected?.mediaIds[selectedSpecialMediaIndex] || ''].filter(Boolean) : selectedBox?.kind === 'media' || selectedBox?.kind === 'image' ? selectedBox.assetIds || [] : selected ? normalizeLayoutBoxes(selected).find(box => box.kind === 'media')?.assetIds || [] : []; const guide = !selected ? '이미지를 먼저 추가한 뒤 캔버스 블록의 이미지 자리를 선택하세요.' : isReference ? selectedSpecialMediaIndex === null ? '가운데 캔버스에서 대표·항목 이미지를 먼저 선택하세요. 선택한 자리만 교체됩니다.' : '선택한 레퍼런스 이미지 자리에 바로 연결됩니다. 파일명·URL은 가운데 이미지에서 확인합니다.' : selectedBox?.kind === 'media' || selectedBox?.kind === 'image' ? '선택한 이미지 박스에 바로 연결됩니다. 파일명·URL은 가운데 이미지에서 확인합니다.' : '선택된 이미지 박스가 없어 기본 이미지 박스에 연결됩니다.'; const externalLibraries = [{ label: 'Getty Images', url: 'https://www.gettyimagesbank.com/' }, { label: 'Pixabay', url: 'https://pixabay.com/ko/' }, { label: 'Pexels', url: 'https://www.pexels.com/ko-kr/' }, { label: 'Unsplash', url: 'https://unsplash.com/ko' }]; return <><div className="panel-heading"><div><p>ASSET LIBRARY</p><h2>이미지</h2></div><span>{assets.length}</span></div><section className="external-image-libraries" aria-label="외부 이미지 라이브러리"><button className="external-image-libraries-toggle" type="button" aria-expanded={librariesOpen} onClick={() => setLibrariesOpen(open => !open)}><span>라이브러리</span>{librariesOpen ? <ChevronUp/> : <ChevronDown/>}</button>{librariesOpen && <div>{externalLibraries.map(library => <button key={library.url} type="button" onClick={() => window.open(library.url, '_blank', 'noopener,noreferrer')}><span>{library.label}</span><ExternalLink/></button>)}</div>}</section><p className="image-library-guide">{guide}</p><button className="upload-zone" onClick={onUpload}><Upload/><strong>이미지 불러오기</strong><small>JPG · PNG · WEBP</small></button><div className="library-url-add"><input aria-label="이미지 URL" type="url" placeholder="https://... 이미지 URL" value={imageUrl} onChange={event => setImageUrl(event.target.value)}/><button disabled={!imageUrl.trim()} onClick={() => { if (imageUrl.trim()) { onAddUrl(imageUrl); setImageUrl('') } }}><Link2/> URL 추가</button></div>{!assets.length ? <div className="empty-state"><ImageIcon/><p>이미지를 올리면 이곳에서<br/>선택 이미지 자리에 바로 연결할 수 있습니다.</p></div> : <div className="asset-grid">{assets.map(asset => <article key={asset.id} className={connectedIds.includes(asset.id) ? 'connected' : ''}><button title={asset.src.startsWith('http') ? asset.src : asset.name} aria-label={`${asset.name} 연결 또는 해제`} onClick={() => onConnect(asset.id)}><img src={asset.src} alt={asset.alt || asset.name}/>{connectedIds.includes(asset.id) && <i><Check/></i>}</button></article>)}</div>}</> }
...
function SectionView({ section, assets, selected, readOnly, activeBoxId, activeItemIndex, activeSpecialMediaIndex, index, total, onSelect, onSelectBox, onSelectItem, onSelectSpecialMedia, onClearTimelineMedia, onSetTimelineHeroVisible, onDeleteTimelineDay, onSectionChange, onBoxChange, onUpdateItem, onAddFlowBox, onMoveFlowBox, onDeleteFlowBox, onAddItem, onAddTimelineDay, onInsertItem, onDuplicateItem, onMoveItem, onDeleteItem, onToggleMenuItemLayout, onMove, onDuplicate, onDelete, onSwapMedia }: { section: Section; assets: MediaAsset[]; selected: boolean; readOnly: boolean; activeBoxId: string; activeItemIndex: number | null; activeSpecialMediaIndex: number | null; index: number; total: number; onSelect: () => void; onSelectBox: (id: string) => void; onSelectItem: (index: number) => void; onSelectSpecialMedia: (index: number) => void; onClearTimelineMedia: (sectionId: string, mediaIndex: number) => void; onSetTimelineHeroVisible: (sectionId: string, visible: boolean) => void; onDeleteTimelineDay: (sectionId: string, dayIndex: number) => void; onSectionChange: (patch: Partial<Section>) => void; onBoxChange: (boxId: string, patch: Partial<BlockBox>) => void; onUpdateItem: (index: number, value: string | string[] | Partial<IconCardItem>) => void; onAddFlowBox: (sectionId: string, kind: 'text' | 'image') => void; onMoveFlowBox: (sectionId: string, boxId: string, direction: -1 | 1) => void; onDeleteFlowBox: (sectionId: string, boxId: string) => void; onAddItem: (sectionId: string) => void; onAddTimelineDay: (sectionId: string) => void; onInsertItem: (sectionId: string, index: number, placement: 'above' | 'below') => void; onDuplicateItem: (sectionId: string, index: number) => void; onMoveItem: (sectionId: string, index: number, direction: -1 | 1) => void; onDeleteItem: (sectionId: string, index: number) => void; onToggleMenuItemLayout: (sectionId: string, index: number) => void; onMove: (id: string, d: -1 | 1) => void; onDuplicate: (id: string) => void; onDelete: (id: string) => void; onSwapMedia: (sectionId: string, fromAssetId: string, toAssetId: string) => void }) {
  ...
  const dragProps = (assetId: string) => ({ draggable: true, onDragStart: (event: React.DragEvent<HTMLElement>) => { event.dataTransfer.effectAllowed = 'move'; event.dataTransfer.setData('text/plain', assetId); event.currentTarget.classList.add('is-dragging') }, onDragEnd: (event: React.DragEvent<HTMLElement>) => event.currentTarget.classList.remove('is-dragging'), onDragOver: (event: React.DragEvent<HTMLElement>) => { event.preventDefault(); event.dataTransfer.dropEffect = 'move'; event.currentTarget.classList.add('is-drop-target') }, onDragLeave: (event: React.DragEvent<HTMLElement>) => event.currentTarget.classList.remove('is-drop-target'), onDrop: (event: React.DragEvent<HTMLElement>) => { event.preventDefault(); event.currentTarget.classList.remove('is-drop-target'); const fromAssetId = event.dataTransfer.getData('text/plain'); if (fromAssetId) onSwapMedia(section.id, fromAssetId, assetId) } })
...
function Inspector({ project, selected, selectedBox, selectedItemIndex, selectedSpecialMediaIndex, commit, updateSection, onAddBox, onDeleteBox, onChangeBox, onChangeBoxLayer, onMoveImageFlowBox, onUpdateItem, onLayoutChange, onLayoutItemChange, onLayoutPreset }: { project: Project; selected?: Section; selectedBox?: BlockBox; selectedItemIndex: number | null; selectedSpecialMediaIndex: number | null; commit: (f: (p: Project) => void) => void; updateSection: (p: Partial<Section>) => void; onAddBox: (sectionId: string, kind: 'text' | 'image') => void; onDeleteBox: (sectionId: string, boxId: string) => void; onChangeBox: (sectionId: string, boxId: string, patch: Partial<BlockBox>) => void; onChangeBoxLayer: (sectionId: string, boxId: string, amount: number) => void; onMoveImageFlowBox: (sectionId: string, boxId: string, direction: -1 | 1) => void; onUpdateItem: (sectionId: string, index: number, value: string | string[] | Partial<IconCardItem>) => void; onLayoutChange: (sectionId: string, layout: Section['mediaLayout']) => void; onLayoutItemChange: (sectionId: string, assetId: string, patch: Partial<MediaLayoutItem>) => void; onLayoutPreset: (sectionId: string, items: MediaLayoutItem[]) => void }) {
...
function ReferenceImageEditor({ section, assets, mediaIndex, onFocusChange }: { section: Section; assets: MediaAsset[]; mediaIndex: number; onFocusChange: (assetId: string, focus: ImageFocus) => void }) {
  ...
  const label = section.type === 'timeline' ? mediaIndex === 0 ? '대표 이미지' : `일정 이미지 ${mediaIndex}` : section.type === 'menu-zigzag' ? `추천 이미지 ${mediaIndex + 1}` : `이미지 ${mediaIndex + 1}`
...
function PresetItemEditor({ section, index, onChange }: { section: Section; index: number; onChange: (value: string | string[] | Partial<IconCardItem>) => void }) {
  ...
  if (section.type === 'icon-card') { const card = section.iconCards?.[index]; if (!card) return null; return <section className="preset-item-editor"><p className="preset-item-kicker">아이콘 카드 {index + 1}</p><div className="two-fields"><Field label="아이콘"><select value={card.icon} onChange={event => onChange({ icon: event.target.value })}>{ICON_CARD_OPTIONS.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select></Field><Field label="강조색"><select value={card.tone} onChange={event => onChange({ tone: event.target.value as IconCardItem['tone'] })}><option value="teal">청록</option><option value="orange">주황</option><option value="green">초록</option></select></Field></div><Field label="카드 제목"><input value={card.title} onChange={event => onChange({ title: event.target.value })}/></Field><Field label="카드 설명"><textarea rows={4} value={card.body} onChange={event => onChange({ body: event.target.value })}/></Field></section> }
  const value = section.items[index] || ''
  if (section.type === 'menu-zigzag') { const item = splitPair(value); const update = (title: string, body: string) => onChange(`${title} | ${body}`); return <section className="preset-item-editor"><p className="preset-item-kicker">추천 항목 {index + 1}</p><Field label="제목"><input value={item.title} onChange={event => update(event.target.value, item.body)}/></Field><Field label="본문"><textarea rows={5} value={item.body} onChange={event => update(item.title, event.target.value)}/></Field></section> }
  if (section.type === 'timeline') { const item = splitTimeline(value); const update = (time: string, title: string, body: string) => onChange(`${time} | ${title} | ${body}`); return <section className="preset-item-editor"><p className="preset-item-kicker">일정 항목 {index + 1}</p><Field label="시간"><input value={item.time} onChange={event => update(event.target.value, item.title, item.body)}/></Field><Field label="제목"><input value={item.title} onChange={event => update(item.time, event.target.value, item.body)}/></Field><Field label="설명"><textarea rows={5} value={item.body} onChange={event => update(item.time, item.title, event.target.value)}/></Field></section> }
  const label = section.type === 'caption-grid' ? '사진 캡션' : section.type === 'offer' ? '특전 또는 조건' : '핵심 내용'
  return <section className="preset-item-editor"><p className="preset-item-kicker">{label} {index + 1}</p><Field label={label}><textarea rows={section.type === 'caption-grid' ? 2 : 4} value={value} onChange={event => onChange(event.target.value)}/></Field></section>
}
